/**
 * Branded HTML email template shared by exam-published and notice-published
 * notifications — same visual system as classroomPost.ts, generalized with
 * a "kind" label instead of being classroom-specific.
 */
export interface NotificationTemplateData {
  studentName: string;
  teacherName: string;
  kindLabel: string; // e.g. "Exam", "Notice"
  title: string;
  detail?: string;
  actionUrl: string;
  actionLabel: string;
...
<head>
  ...
  <title>New ${escapeHtml(kindLabel)}: ${escapeHtml(title)}</title>
  <style>
      ...
      <td>
        <div class="container">
          <div class="header">
            ...
            <div class="sub">${escapeHtml(kindLabel)} Notification</div>
          </div>
          <div class="body">
            <div class="greeting">Hi ${escapeHtml(studentName)},</div>
            <p class="message">${escapeHtml(teacherName)} published a new ${escapeHtml(kindLabel.toLowerCase())} for you.</p>
            <div class="card">
              <span class="badge">${escapeHtml(kindLabel)}</span>
              <div class="card-title">${escapeHtml(title)}</div>
              <div class="card-sub">${detail ? `${escapeHtml(detail)} &middot; ` : ""}${dateStr}</div>
            </div>
            <div style="text-align:center;">
              <a class="button" href="${escapeHtml(actionUrl)}">${escapeHtml(actionLabel)}</a>
            </div>
          </div>
          <div class="footer">
